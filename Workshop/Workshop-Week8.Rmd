...
We now look at the distribution in the case that $\rho = 0$ by sample $N$ observations from a $N_2(\mu, I)$ distribution.
...
We look at the population multiple correlation coefficient between $X_1$ and $\mathbb{X}_2 = (X_2, X_3, \ldots, X_p)'$. Let's look at a simple example where $p=5$. Generate a random covariance matrix.
...
\end{pmatrix}.
$$
The multiple correlation coefficient between $X_1$ and $\X_2$ is
$$
\rho =
...
par(opar)
```
Notice that the theorem in the lecture doesn't apply to the adjusted R-squared given by $R^*$.
...
We can see that the quantity $R^2 - (1-y)\rho^2 - y$ is closely distributed around zero, or in other words, $R^2$ does not converge to $\rho^2$ as desired! So in the high-dimensional case, $R^2 \to (1-y)\rho^2 + y$ instead compared to the classic theory that states $R^2 \to \rho^2$.
```{r}
mean(MC)
```
